...
use crate::tui::app::TuiJobInfo;

// ─────────────────────────────────────────────────────────────────────────────
// Job Detail Section Builders
// ─────────────────────────────────────────────────────────────────────────────

/// Build the header section: state, user/account, partition/QOS
fn build_job_header_section<'a>(job: &'a TuiJobInfo, theme: &Theme) -> Vec<Line<'a>> {
    ...
    vec![
        ...
        Line::from(vec![
            ...
            Span::raw(job.user_name.as_str()),
            Span::raw("    "),
            Span::styled("Account: ", Style::default().bold()),
            Span::styled(job.account.as_str(), Style::default().fg(theme.account_highlight)),
        ]),
        Line::from(vec![
            Span::styled("  Partition:   ", Style::default().bold()),
            Span::raw(job.partition.as_str()),
            Span::raw("    "),
            Span::styled("QOS: ", Style::default().bold()),
            Span::raw(job.qos.as_str()),
        ]),
    ]
}

/// Build the time information section
fn build_time_section(job: &TuiJobInfo, theme: &Theme) -> Vec<Line<'static>> {
    let mut lines = vec![
        Line::from(""),
        section_header("Time Information", theme),
        Line::from(vec![
...
/// Build the resources section: CPUs, memory, GPUs, nodes
fn build_resources_section(job: &TuiJobInfo, theme: &Theme) -> Vec<Line<'static>> {
    let mut lines = vec![
        Line::from(""),
        section_header("Resources", theme),
        // CPUs
        Line::from(vec![
    ...
    // GPUs
    if job.gpu_count > 0 {
        let gpu_type_span = job
            .gpu_type
            .as_ref()
            .map(|t| Span::styled(format!(" ({})", t), Style::default().fg(theme.border)))
...
/// Build the paths section: working dir, stdout, stderr
fn build_paths_section(job: &TuiJobInfo, theme: &Theme) -> Vec<Line<'static>> {
    let mut lines = vec![
        Line::from(""),
        section_header("Paths", theme),
        Line::from(vec![
...
/// Build the extras section: dependencies, array job info, priority, constraint
fn build_extras_section(job: &TuiJobInfo, theme: &Theme) -> Vec<Line<'static>> {
    ...
    if !job.constraint.is_empty() {
        lines.push(Line::from(vec![
            ...
            Span::styled(job.constraint.clone(), Style::default().fg(theme.border)),
        ]));
    }
...
fn build_footer_section(theme: &Theme) -> Vec<Line<'static>> {
    vec![
        Line::from(""),
        Line::from(vec![
...
pub fn render_job_detail_popup(app: &App, frame: &mut Frame, area: Rect, theme: &Theme) {
    ...
    // Build content using section helpers
    let mut lines: Vec<Line<'_>> = Vec::new();
    lines.extend(build_job_header_section(job, theme));
    lines.extend(build_time_section(job, theme));
    lines.extend(build_resources_section(job, theme));
    lines.extend(build_paths_section(job, theme));
    lines.extend(build_extras_section(job, theme));
    lines.extend(build_footer_section(theme));
